...
pub fn expression_to_ast(context: &Context, expression: &il::Expression) -> Result<Ast> {
    Ok(match *expression {
        ...
        il::Expression::AShr(ref lhs, ref rhs) => context.bvashr(
            &expression_to_ast(context, lhs)?,
            &expression_to_ast(context, rhs)?,
        )?,
...
fn test_solve() -> Result<()> {
    ...
    assert_eq!(solve(&[], &expression)?.unwrap(), il::const_(32, 32));

    let expression = il::expr_const(0x1000, 32);
    assert_eq!(solve(&[], &expression)?.unwrap(), il::const_(0x1000, 32));

    let expression = il::expr_const(0x12345678, 32);
    assert_eq!(
        solve(&[], &expression)?.unwrap(),
        il::const_(0x12345678, 32)
    );

    let expression = il::Expression::add(il::expr_const(32, 32), il::expr_const(1, 32))?;
    assert_eq!(solve(&[], &expression)?.unwrap(), il::const_(33, 32));

    let expression =
        il::Expression::add(il::expr_const(0x420000, 32), il::expr_const(0xffffc000, 32))?;
    assert_eq!(solve(&[], &expression)?.unwrap(), il::const_(0x41c000, 32));

    let expression = il::Expression::add(
        il::Expression::add(il::expr_const(0x420000, 32), il::expr_const(0xffffc000, 32))?,
        il::expr_const(0xffff83dc, 32),
    )?;
    assert_eq!(solve(&[], &expression)?.unwrap(), il::const_(0x4143dc, 32));
