...
fn optimize_init(
    ...
    for constraint in constraints {
        optimize.assert(&context.eq(&one,
                                    &expression_to_ast(&context, constraint)?));
    }
...
pub fn maximize(constraints: &[il::Expression], value: &il::Expression)
    -> Result<Option<il::Constant>> {
    ...
    let optimize_result = context.mk_var("OPTIMIZE_RESULT",
                                         &context.mk_bv_sort(value.bits()))?;

    optimize.assert(&context.eq(&optimize_result,
...
pub fn minimize(constraints: &[il::Expression], value: &il::Expression)
    -> Result<Option<il::Constant>> {
