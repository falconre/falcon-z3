...
impl<'c> Optimize<'c> {
    ...
    pub fn assert(&self, term: &Ast) {
        unsafe {
           z3_sys::Z3_optimize_assert(self.context.context,
                                      self.optimize,
                                      term.ast);
        }
    }
    ...
    pub fn check(&self) -> Check {
        let lbool = unsafe {
            z3_sys::Z3_optimize_check(self.context.context, self.optimize)
        };
        match lbool {
    ...
    pub fn maximize(&self, term: &Ast) {
        unsafe {
           z3_sys::Z3_optimize_maximize(self.context.context,
                                        self.optimize,
                                        term.ast);
        }
    }
    ...
    pub fn minimize(&self, term: &Ast) {
        unsafe {
           z3_sys::Z3_optimize_minimize(self.context.context,
                                        self.optimize,
                                        term.ast)
        };
    }
